  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <div id="container" style="margin-top: -10%;">
        ...
        <h4 style="color: #8cb3f4;margin-top:-20%;">CONNECTER VOUS.</h4>
        <small>{{message}}</small>
        <ion-card style="margin: 20px;">
          <form v-on:submit.prevent="LogIn">
            <ion-item>
              <ion-input type="text" v-model="username" name="username" placeholder="Utilisateur" value="" required></ion-input>
            </ion-item>
            <ion-item>
              <ion-input type="password" v-model="password" name="password" placeholder="Mot de Passe" value="" required></ion-input>
            </ion-item>
...
export default defineComponent({
  ...
  methods: {
    ...
    // },
    LogIn: function(){
      // send a request to the server to get the user info if they are valid
       if(this.username == '+243996848331' && this.password == 'client'){
            this.username = '';
            this.password = '';
              // const userInfo = {
              ...
              //     };
            // localStorage.setItem('config',JSON.stringify(userInfo));
            this.message = 'Connexion avec success';
            var owner = localStorage.getItem('adminConfig');
            if(owner == null){
              this.redirect('home');
            }else{
              var user = JSON.parse(owner);
              if(user.posType == 'recharge'){
                this.redirect('recharge');
              }else if(user.posType == 'register'){
                this.redirect('register');
              }else if(user.posType == 'payment'){
                this.redirect('pos');
              }
            }
        } else{
